...
for (count, currdirname) in enumerate(dirlist)
	...
	currimg = Pyehtim.ehtim.image.load_fits(joinpath(inbase , currdirname )).blur_circ(μas2rad(10.0f0))
	...
	param_file1 = abspath(joinpath((@__DIR__) , "..", "..", "..", "runs", "image_domain" , currdirname , "EqBlur", "best_nxcorr.txt"))
	best_fit = plot_best_fit(param_file1, eq_blur_temp, ax2)
	param_file2 = abspath(joinpath((@__DIR__) , "..", "..", "..", "runs", "image_domain" , currdirname , "JBOXBlur", "best_nxcorr.txt"))
	best_fit = plot_best_fit(param_file2, dual_cone_blur_temp, ax3)
	param_file3 = abspath(joinpath((@__DIR__) , "..", "..", "..", "runs", "image_domain" , currdirname , "EqDualConeBlur", "best_nxcorr.txt"))
	best_fit = plot_best_fit(param_file3, eq_dual_cone_blur_temp, ax4)
...
models = ["EqBlur", "EqDualConeBlur", "JBOXBlur"]#, "EqDualConeBlur"]
...
for (inc, dir) in zip(inclinations, directory_names)
	for model in models
		...
		append!(nxcorr_vals[model], [parse(Float64, match(r"0[.][0-9]*",readline(f)).match)])
		close(f)
	end
end
...
save(joinpath((@__DIR__), "nxcorr_blur_summary.pdf"), fig)
