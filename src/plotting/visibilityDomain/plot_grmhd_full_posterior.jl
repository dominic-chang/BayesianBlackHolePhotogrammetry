...
fig = Figure();
ax = Axis(fig[1,1])
CairoMakie.scatter!(ax, mode1.value[:,1:2,1])
CairoMakie.scatter!(ax, mode2.value[:,1:2,1])
display(fig)
hpd(mode1)
hpd(mode2)

save(joinpath((@__DIR__), "$(model_name)_full_posterior.pdf"), plt)
