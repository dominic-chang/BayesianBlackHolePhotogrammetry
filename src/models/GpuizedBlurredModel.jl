...
@inline function ComradeBase.visibility_point(m::GPUBlurModel, u, v, time, freq) ComradeBase.visibility_point(m.model, u, v, time, freq) end
@inline function ComradeBase.intensity_point(m::GPUBlurModel, p)  ComradeBase.intensity_point(m.model, p) end
...
ComradeBase.intensitymap!(img::IntensityMapTypes, m, ::VLBISkyModels.True)  = intensitymap!(img, GPUBlurModel(m))

function ComradeBase.intensitymap_analytic!(img::ComradeBase.IntensityMap, s::GPUBlurModel)
    ...
    #return imfilter(img, Kernel.gaussian(s.blur/(2*√(dx*dy*2log(2f0)))))
end
