...
struct ModelInfo{M, T, F} <: AbstractModelInfo
    ...
    function ModelInfo(model::M, name::String, lower::NamedTuple{T}, upper::NamedTuple{T}, file::String, blur::F=zero(F))  where{M, T,F}
        ...
        if !isnothing(check)
            ...
            lower = NamedTuple{keys(lower)}([i == :θo ? max((inc - 20f0)/180f0*π,0f0) : lower[i] for i in keys(lower)])
            upper = NamedTuple{keys(upper)}([i == :θo ? max((inc + 20f0)/180f0*π,180f0) : upper[i] for i in keys(upper)])
        end
